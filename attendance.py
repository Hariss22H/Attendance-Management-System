...
def TakeImageUI():
    ...
    def open_captured_images():
        if not os.path.exists(trainimage_path):
            os.makedirs(trainimage_path)
        os.startfile(trainimage_path)

    checkImg = tk.Button(
        ImageUI,
        text="Check Images",
        command=open_captured_images,
        bd=10,
        font=("times new roman", 18),
        bg="black",
        fg="yellow",
    ...
    checkImg.place(x=130, y=430)

    def open_trainer_file():
        trainer_dir = os.path.dirname(trainimagelabel_path)
        if not os.path.exists(trainer_dir):
            os.makedirs(trainer_dir)
        if os.path.exists(trainimagelabel_path):
            os.startfile(trainimagelabel_path)
        else:
            message.configure(
                text="Train images first to create the model.",
            )
            text_to_speech("Train images first to create the model.")

    checkTrainer = tk.Button(
        ImageUI,
        text="Check Trained Model",
        command=open_trainer_file,
        bd=10,
        font=("times new roman", 18),
        bg="black",
        fg="yellow",
    ...
    checkTrainer.place(x=360, y=430)
